...
public class Pmac02Entity extends PmaBaseEntity {
    ...
    protected void registerCombatActions() {
        ...
        this.actionController.registerAction(ACT_MISSILE, new Action(60, 5, 10), ActionController.ActionType.L_SHL_MAIN);
    }
...
//                this.actionController.getAction(ACT_MISSILE).startAction();
    ...
    private void fireShoot(Level level) {
        if (!level.isClientSide()) {
            ...
            var muzzlPos = new Vec3(-3F/2, 11F/2, 25F/2);
    ...
    private void fireBazooka(Level level) {
        if (!level.isClientSide()) {
            for (int i = 0; i < 3; i+=2) {
                ...
                // オフセット位置から大体の銃口の座標を決める（モデル位置からとるとクラサバ同期がめんどい…）
                var muzzlPos = new Vec3((i-1) * 3F/2, 8F/2, 12F/2);
                muzzlPos = muzzlPos.yRot((float) Math.toRadians((-1.0) * this.getYRot()));
